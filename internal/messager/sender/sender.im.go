...
type Im interface {
	Send(model.ImMessageInfo) (err error)
}
...
func NewImSender(ctx context.Context) (m Im, err error) {
	m = &imSender{
		ctx:       ctx,
		maxWorker: 1,
	}
...
func (m *imSender) Send(mi model.ImMessageInfo) (err error) {
	// sender info
	...
	p, err := provider.GetImFactory().Create(si.Provider, string(si.Config))
	if err != nil {

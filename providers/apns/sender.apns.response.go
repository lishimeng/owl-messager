...
const (
	// ReasonBadCollapseID 400 The collapse identifier exceeds the maximum allowed size
	ReasonBadCollapseID = "BadCollapseId"

	// ReasonBadDeviceToken 400 The specified device token was bad. Verify that the request contains a
	// valid token and that the token matches the environment.
	ReasonBadDeviceToken = "BadDeviceToken"

	// ReasonBadExpirationDate 400 The apns-expiration value is bad.
	ReasonBadExpirationDate = "BadExpirationDate"

	// ReasonBadMessageID 400 The apns-id value is bad.
	ReasonBadMessageID = "BadMessageId"

	// ReasonBadPriority 400 The apns-priority value is bad.
	ReasonBadPriority = "BadPriority"

	// ReasonBadTopic 400 The apns-topic was invalid.
	ReasonBadTopic = "BadTopic"

	// ReasonDeviceTokenNotForTopic 400 The device token does not match the specified topic.
	ReasonDeviceTokenNotForTopic = "DeviceTokenNotForTopic"

	// ReasonDuplicateHeaders 400 One or more headers were repeated.
	ReasonDuplicateHeaders = "DuplicateHeaders"

	// ReasonIdleTimeout 400 Idle time out.
	ReasonIdleTimeout = "IdleTimeout"

	// ReasonInvalidPushType 400 The apns-push-type value is invalid.
	ReasonInvalidPushType = "InvalidPushType"

	// ReasonMissingDeviceToken 400 The device token is not specified in the request :path. Verify that the
	// :path header contains the device token.
	ReasonMissingDeviceToken = "MissingDeviceToken"

	// ReasonMissingTopic 400 The apns-topic header of the request was not specified and was
	...
	// ReasonPayloadEmpty 400 The message payload was empty.
	ReasonPayloadEmpty = "PayloadEmpty"

	// ReasonTopicDisallowed 400 Pushing to this topic is not allowed.
	ReasonTopicDisallowed = "TopicDisallowed"

	// ReasonBadCertificate 403 The certificate was bad.
	ReasonBadCertificate = "BadCertificate"

	// ReasonBadCertificateEnvironment 403 The client certificate was for the wrong environment.
	ReasonBadCertificateEnvironment = "BadCertificateEnvironment"

	// ReasonExpiredProviderToken 403 The provider token is stale and a new token should be generated.
	ReasonExpiredProviderToken = "ExpiredProviderToken"

	// ReasonForbidden 403 The specified action is not allowed.
	ReasonForbidden = "Forbidden"

	// ReasonInvalidProviderToken 403 The provider token is not valid or the token signature could not be
	// verified.
	ReasonInvalidProviderToken = "InvalidProviderToken"

	// ReasonMissingProviderToken 403 No provider certificate was used to connect to APNs and Authorization
	// header was missing or no provider token was specified.
	ReasonMissingProviderToken = "MissingProviderToken"

	// ReasonBadPath 404 The request contained a bad :path value.
	ReasonBadPath = "BadPath"

	// ReasonMethodNotAllowed 405 The specified :method was not POST.
	ReasonMethodNotAllowed = "MethodNotAllowed"

	// ReasonUnregistered 410 The device token is inactive for the specified topic.
	ReasonUnregistered = "Unregistered"

	// ReasonPayloadTooLarge 413 The message payload was too large. See Creating the Remote Notification
	...
	// ReasonTooManyProviderTokenUpdates 429 The provider token is being updated too often.
	ReasonTooManyProviderTokenUpdates = "TooManyProviderTokenUpdates"

	// ReasonTooManyRequests 429 Too many requests were made consecutively to the same device token.
	ReasonTooManyRequests = "TooManyRequests"

	// ReasonInternalServerError 500 An internal server error occurred.
	ReasonInternalServerError = "InternalServerError"

	// ReasonServiceUnavailable 503 The service is unavailable.
	ReasonServiceUnavailable = "ServiceUnavailable"

	// ReasonShutdown 503 The server is shutting down.

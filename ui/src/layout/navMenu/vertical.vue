		...
		<template v-for="val in menuLists">
			<el-sub-menu :index="val.path" v-if="val.children && val.children.length > 0" :key="val.path">
				<template #title>
					...
					<span>{{ val.meta.i18nTitle ? proxy.$i(val.meta.i18nTitle) : $t(val.meta.title) }}</span>
				</template>
			...
			<el-menu-item :index="val.path" :key="val.path" v-else>
				...
				<template #title v-else>
					<a :href="val.meta.isLink" target="_blank" rel="opener">{{ val.meta.i18nTitle ? proxy.$i(val.meta.i18nTitle) : $t(val.meta.title) }}</a></template
				>
			</el-menu-item>
		</template>
	</el-menu>

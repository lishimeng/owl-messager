		...
		onMounted(() => {
			nextTick(() => {
				...
				axios.get('static/config.json').then((res)=>{
					// Local.set("JsonConfigs", res.data);
					store.dispatch('jsonConfigs/setJsonConfigs', res.data);
				}).catch(err => {
					console.log("err config: " + err);
				})
				// i18n资源
				axios.get('static/i18n_zh_cn.json').then((res)=>{
					store.dispatch('sysI18nSource/setSysI18nSource', res.data);
				}).catch(err => {
					console.log("err locading static/i18n_zh_cn.json: " + err);
				})
			});
		});
